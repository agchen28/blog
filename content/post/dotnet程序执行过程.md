...
- 调用虚实例方法  
JIT编译器会在方法中生成一些额外的代码，方法每次调用都会执行这些代码（实际上就是做的动态绑定的工作，将方法调用与方法主体关联起来被称为绑定，虚实例方法只有在运行时才能确定方法调用的主体所以只能通过动态绑定）。  
...
类型对象不会被回收（这个是我猜的，书上没看到明确的说明），类型对象只有在第一次使用到该类型的时候才进行一次也是唯一一次构造。