...
不过出问题的概率极低，关键是大多数时候他们需要的并不是一个真正的单例，所以即使这么写也不会发现有什么问题。
...
总之在这边就可以这么理解：在lock之后（相当于java中的synchronized），uniqueInstance字段的值必须重新去内存中读取，CPU寄存器里面的东西做不了数。
